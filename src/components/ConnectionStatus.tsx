  ...
  return (
    <div className="flex items-center space-x-3">
      ...
      <div className="flex items-center space-x-2">
        {/* Backend Status */}
        <div className="flex items-center space-x-1" aria-label={`Backend: ${backendConnected ? 'Connected' : 'Disconnected'}`} title={`Backend: ${backendConnected ? 'Connected' : 'Disconnected'}`}>
          <div
          ...
          />
          {variant === 'full' && (
            <span className="text-xs text-gray-500 dark:text-gray-400 hidden sm:inline">Backend</span>
          )}
        </div>

        {/* Radio Status */}
        <div className="flex items-center space-x-1" aria-label={`Radio: ${radioConnected ? 'Connected' : 'Disconnected'}`} title={`Radio: ${radioConnected ? 'Connected' : 'Disconnected'}`}>
          <div
            className={`w-2 h-2 rounded-full ${
          ...
          />
          {variant === 'full' && (
            <span className="text-xs text-gray-500 dark:text-gray-400 hidden sm:inline">Radio</span>
          )}
        </div>
      </div>
